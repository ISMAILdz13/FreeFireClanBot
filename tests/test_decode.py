...
class TestPacketTypeSummary(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_scan_multiple_packets(self):
        """Scan a hex stream with multiple distinct packets and find all f2 values.
        Uses larger packets with nested fields so each packet is long enough
        for the 40-char scan jump to land between packets."""
        # Build larger packets with nested structures (simulates real server data)
        packets = []
        for f2_val in [5, 18, 30]:
            fields = {
                1: 12345,
                2: f2_val,
                3: {1: 100001, 2: 20, 3: f2_val},
                4: "device_info_padding_here_to_make_packet_longer",
            }
            proto = await CrEaTe_ProTo(fields)
            packets.append(proto.hex())

        # Scan each packet individually (as the scanner would in real data)
        found_types = {}
        for pkt_hex in packets:
            try:
                json_str = await DeCode_PackEt(pkt_hex)
                if json_str:
                    parsed = json.loads(json_str)
                    f2 = parsed.get('2', {})
                    f2_val = f2.get('data') if isinstance(f2, dict) else f2
        ...
        self.assertGreater(len(found_types), 0, "No packet types found")
        self.assertIn(18, found_types, "f2=18 not found in scanned packets")
        self.assertIn(5, found_types, "f2=5 not found")
        self.assertIn(30, found_types, "f2=30 not found")
