...
class TestAESEncryption(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_aes_known_key_iv(self):
        ...
        # Key = "Yg&tc%DEuh6%Zc^8" as bytes
        self.assertEqual(Key, b"Yg&tc%DEuh6%Zc^8")
        # Iv = "6oyZDr22E3ychjM%" as bytes
...
class TestProtobufEncoding(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_simple_fields(self):
        ...
        self.assertIsInstance(proto, bytes)
    ...
    async def test_encode_decode_round_trip(self):
        ...
        # Field 2 should contain field 1 = UID
...
class TestPacketGeneration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_packet_structure(self):
        ...
        # Header should start with packet type
        packet_hex = packet.hex()
        self.assertTrue(packet_hex.startswith("0515"))

    async def test_packet_decodable(self):
        """Generated packet payload should be decryptable."""
        key = bytes([0x41] * 16)
        iv = bytes([0x42] * 16)
        fields = {1: 9, 2: {1: 16648969335}}
        proto = await CrEaTe_ProTo(fields)
        packet_hex = await GeneRaTePk(proto.hex(), "0515", key, iv)
        # Extract payload (skip header — header length varies)
        # Header format: packet_type (2 bytes) + length (variable) + encrypted payload
        # For small packets, length is 1-2 bytes
        # Find the encrypted payload by skipping the header
        # The header is: 2 bytes type + N bytes length + payload
        # We can try to decrypt the payload portion
        # First, extract just the encrypted portion
        # Header = "0515" + "0000" + length_bytes
        # For a small packet, total header = 8-10 hex chars
        for header_len in [8, 10, 12]:
            payload_hex = packet_hex[header_len:]
            if len(payload_hex) % 2 == 0 and len(payload_hex) > 0:
                try:
                    decrypted = await DEc_PacKeT(payload_hex, key, iv)
                    ...
                    continue
        self.fail("Could not decode generated packet")
...
class TestHexDecode(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_large_numbers(self):
        """Large numbers should produce multi-byte hex."""
...
class TestVersionConsistency(unittest.TestCase):
    ...
    def test_clan_bot_version(self):
        ...
        self.assertIn(self.EXPECTED_VERSION, content,
                      f"Version {self.EXPECTED_VERSION} not found in clan_glory_bot.py")
