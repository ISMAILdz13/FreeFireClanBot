...
def CrEaTe_LenGTh_sync(field_number, value):
    return EnC_Vr_sync((field_number << 3) | 2) + EnC_Vr_sync(len(value)) + value

def CrEaTe_ProTo_sync(fields):
    packet_data = b""
    for field_number, value in fields.items():
        if isinstance(value, int):
            packet_data += CrEaTe_VarianT_sync(field_number, value)
        elif isinstance(value, str):
            packet_data += CrEaTe_LenGTh_sync(field_number, value.encode("utf-8"))
        elif isinstance(value, bytes):
            packet_data += CrEaTe_LenGTh_sync(field_number, value)
        elif isinstance(value, dict):
            packet_data += CrEaTe_LenGTh_sync(field_number, CrEaTe_ProTo_sync(value))
    return packet_data
